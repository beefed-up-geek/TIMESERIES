# 경로: ./data_provider/data_loader.py
import os
import numpy as np
import pandas as pd
...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        # target=None이면 모든 컬럼을 target으로 설정
        if self.target is None:
            self.target = cols.copy()

        # 입력과 출력 데이터를 명확히 분리 (모든 컬럼 사용)
        df_data = df_raw[cols]
        ...
        if 'date' in df_raw.columns:
            ...
            # date 컬럼이 없을 경우, 단순 인덱스 기반의 timestamp로 처리
            data_stamp = np.expand_dims(np.arange(border1, border2), axis=1)

        self.data_x = data[border1:border2]
        self.data_y = data[border1:border2]  # 모든 데이터를 예측 대상으로
        self.data_stamp = data_stamp
...
class Dataset_PEMS(Dataset):
    ...
    def __read_data__(self):
        ...
        num_vali = int(num_samples * 0.1)
        num_test = num_samples - num_train - num_vali
        ...
        data = df_raw.values  # 모든 컬럼을 사용 (모두 타겟)
        ...
        self.data_x = data[border1:border2]
        self.data_y = data[border1:border2]  # 모든 컬럼을 타겟으로 지정
    ...
    def __read_data__(self):
        data_npz = np.load(os.path.join(self.root_path, self.data_path))
        data_array = data_npz['data']  # (time_steps, num_sensors, num_features)

        time_steps = data_array.shape[0]
        num_train = int(time_steps * 0.7)
        num_test = int(time_steps * 0.2)
        num_vali = time_steps - num_train - num_test
        border1s = [0, num_train - self.seq_len, time_steps - num_test - self.seq_len]
        border2s = [num_train, num_train + num_vali, time_steps]
        ...
        # 데이터를 2차원으로 변형: (time_steps, num_sensors*num_features)
        self.data = data_array.reshape(time_steps, -1)
